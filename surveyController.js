...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var ques1 = readData("ques1");
        var ques2 = readData("ques2")
        var ques3 = readData("ques3")
        var ques4 = readData("ques4")
        var ques5 = readData("ques5")
        res.render('showResults', {results: [answers]});
        console.log([answers]);
    });

    // when a user goes to localhost:3000/niceSurvey
    // serve a static html (the survey itself to fill in)
    app.get('/webSurvey', function(req, res){
        res.sendFile(__dirname+'/views/webSurvey.html');
    });
    ...
    app.post('/webSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "ques4") && (json[key].length === 2)){
